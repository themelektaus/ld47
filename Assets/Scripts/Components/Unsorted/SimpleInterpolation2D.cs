		...
		void OnUpdate() {
			...
			if (includeRotation) {
				if (rotationSmoothness == 0) {
					...
					var eulerAngles = source.eulerAngles;
					eulerAngles.z = Mathf.SmoothDampAngle(eulerAngles.z, target.eulerAngles.z, ref rotationVelocity, rotationSmoothness);
					source.eulerAngles = eulerAngles;
				}
			}
		}
